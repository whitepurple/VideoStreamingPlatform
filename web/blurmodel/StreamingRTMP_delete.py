...
class StreamingRTMP:
    ...
    def updateEmbedding(self):
        ...
        embedding = []
        registerdEmbedding = np.full((3,3072),100, dtype=np.float32)
        for f in faces:
            frame = (
                np
        ...
        if len(embedding) != 0:
            raw = np.array(embedding)
            registerdEmbedding = raw.reshape(-1, raw.shape[2])
    ...
    def faceCrop(self, frame, box):
        ...
        face_crop = cv2.resize(frame[box[1]:box[3],box[0]:box[2]],(128,128),cv2.INTER_AREA)
    ...
    def faceblur2(self,frame, box, landmark):
        ...
        blur_img = cv2.resize(self.straight,(face.shape[1],face.shape[0]))    
